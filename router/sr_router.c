...
uint16_t get_tcp_checksum(sr_tcp_hdr_t *tcp, uint32_t ip_src, uint32_t ip_dst, uint16_t tcp_len) {
    uint8_t *full_tcp;
    sr_tcp_cksum_hdr_t *tcp_cksum = (sr_tcp_cksum_hdr_t *) malloc(sizeof(sr_tcp_cksum_hdr_t));
    tcp_cksum->tcp_src_ip = ip_src;
    tcp_cksum->tcp_dst_ip = ip_dst;
    tcp_cksum->tcp_zero = 0;
    tcp_cksum->tcp_pro = ip_protocol_tcp;
    tcp_cksum->tcp_len = htons(tcp_len);

    uint16_t temp = tcp->checksum;
    tcp->checksum = 0;
    full_tcp = malloc(sizeof(sr_tcp_cksum_hdr_t) + tcp_len);
    memcpy(full_tcp, (uint8_t *) tcp_cksum, sizeof(sr_tcp_cksum_hdr_t));
    memcpy(&(full_tcp[sizeof(sr_tcp_cksum_hdr_t)]), (uint8_t *) tcp, tcp_len);
    tcp->checksum = temp;
    
    /* Calculating the result checksum */
    uint16_t result = cksum(full_tcp, sizeof(sr_tcp_cksum_hdr_t) + tcp_len);
...
void send_icmp_packet(struct sr_instance *sr, uint8_t type, uint8_t icmp_code, struct sr_if *interface, uint8_t *packet) {
    uint8_t *eth_icmp_pkt = create_ethernet_header(4, interface->addr, NULL, 0);
    ...
    if ((arpentry = sr_arpcache_lookup(&sr->cache, ip_header->ip_src)) != NULL) {
        /*arp cache hit*/
        printf("ARP cache is hit\n");
        memcpy(((sr_ethernet_hdr_t *) eth_icmp_pkt)->ether_dhost, arpentry->mac, ETHER_ADDR_LEN);
        sr_send_packet(sr, eth_icmp_pkt, sizeof(sr_ethernet_hdr_t) + ip_len, interface->name);
    } else {
        /*arp cache miss*/
        printf("ARP cache miss\n");
        struct sr_arpreq *newarpreq = sr_arpcache_queuereq(&sr->cache, ip_header->ip_src, eth_icmp_pkt, sizeof(sr_ethernet_hdr_t) + ip_len, interface->name);
		...
		char* interface)
{
	...
	if (fetch_interface == NULL) {
        ...
        if (ip_hdr->ip_ttl <= 0) {
            ...
            if (sr->nat != NULL) {
                /* Packet is going from internal to external */
                struct sr_nat_mapping *nat_map;
                /* ICMP packet */
                if (ip_proto == ip_protocol_icmp) {
                    sr_icmp_echo_hdr_t *icmp_echo = (sr_icmp_echo_hdr_t *) (packet + sizeof(sr_ethernet_hdr_t) + sizeof(sr_ip_hdr_t));  
                    if ((nat_map = sr_nat_lookup_internal(sr->nat, ip_hdr->ip_src, icmp_echo->icmp_id, nat_mapping_icmp)) == NULL) {
                        nat_map = sr_nat_insert_mapping(sr->nat, ip_hdr->ip_src, icmp_echo->icmp_id, nat_mapping_icmp);
                    }
                    print_nat_mapping(nat_map);        
                    ip_hdr->ip_src = nat_map->ip_ext;
                    icmp_echo->icmp_id = nat_map->aux_ext;
                    ip_hdr->ip_sum = ip_cksum(ip_hdr, sizeof(sr_ip_hdr_t));
                    int total_len = len - sizeof(sr_ethernet_hdr_t) - sizeof(sr_ip_hdr_t); 
                    icmp_echo->icmp_sum = icmp_cksum((sr_icmp_hdr_t *) icmp_echo, total_len);
                /* TCP packet */
                } else if (ip_proto == ip_protocol_tcp) {
                    sr_tcp_hdr_t *tcp = (sr_tcp_hdr_t *) (packet + sizeof(sr_ethernet_hdr_t) + sizeof(sr_ip_hdr_t));
                    if ((nat_map = sr_nat_lookup_internal(sr->nat, ip_hdr->ip_src, tcp->tcp_src, nat_mapping_tcp)) == NULL) {
                        nat_map = sr_nat_insert_mapping(sr->nat, ip_hdr->ip_src, tcp->tcp_src, nat_mapping_tcp);
                    }
                    ip_hdr->ip_src = nat_map->ip_ext;
                    tcp->tcp_src = nat_map->aux_ext;
                    ip_hdr->ip_sum = ip_cksum(ip_hdr, sizeof(sr_ip_hdr_t));
                            
                    uint16_t tcp_len = len - sizeof(sr_ethernet_hdr_t) - sizeof(sr_ip_hdr_t);
                    tcp->checksum = get_tcp_checksum(tcp, nat_map->ip_ext, ip_hdr->ip_dst, tcp_len); 
        ...
        if (sr->nat != NULL) {
            /* Packet is going from external to internal */
            struct sr_nat_mapping *nat_map;
            if(ip_proto == ip_protocol_icmp) {
                sr_icmp_echo_hdr_t *icmp_echo = (sr_icmp_echo_hdr_t *) (packet + sizeof(sr_ethernet_hdr_t) + sizeof(sr_ip_hdr_t));
                if ((nat_map = sr_nat_lookup_external(sr->nat, icmp_echo->icmp_id, nat_mapping_icmp)) != NULL) {
                    ip_hdr->ip_dst = nat_map->ip_int;
                    icmp_echo->icmp_id = nat_map->aux_int;
                } 
                ip_hdr->ip_sum = ip_cksum(ip_hdr, sizeof(sr_ip_hdr_t));
                int total_len = len - sizeof(sr_ethernet_hdr_t) - sizeof(sr_ip_hdr_t); 
                icmp_echo->icmp_sum = icmp_cksum((sr_icmp_hdr_t *) icmp_echo, total_len);
                ...
                sr_tcp_hdr_t *tcp = (sr_tcp_hdr_t *) (packet + sizeof(sr_ethernet_hdr_t) + sizeof(sr_ip_hdr_t));
                if ((nat_map = sr_nat_lookup_external(sr->nat, tcp->tcp_dst, nat_mapping_tcp)) != NULL) {
                    ip_hdr->ip_dst = nat_map->ip_int;
                    tcp->tcp_dst = nat_map->aux_int;
                    ip_hdr->ip_sum = ip_cksum(ip_hdr, sizeof(sr_ip_hdr_t));
                    uint16_t tcp_len = len - sizeof(sr_ethernet_hdr_t) - sizeof(sr_ip_hdr_t);           
                    tcp->checksum = get_tcp_checksum(tcp, ip_hdr->ip_src, nat_map->ip_int, tcp_len);
                } else {
                    /* Received an unsolicited tcp? */
                    if (ntohs(tcp->tcp_dst) >= 1024) {
                        insert_tcp_unsol(sr->nat, sr, incoming_interface, packet, tcp->tcp_src);                       
                    } else {
                    /* Just send ICMP message immidiately */
            ...
            if ((arpentry = sr_arpcache_lookup(&sr->cache, ip_hdr->ip_dst)) != NULL) {
                /*arp cache hit*/
                printf("ARP cache is hit\n");
                struct sr_if *pkt_iface = sr_get_interface(sr, "eth1");
                memcpy(ether_hdr->ether_shost, pkt_iface->addr, ETHER_ADDR_LEN);
                memcpy(ether_hdr->ether_dhost, arpentry->mac, ETHER_ADDR_LEN);               
                sr_send_packet(sr, packet, len, pkt_iface->name);
            } else {
                /*arp cache miss*/
                printf("ARP cache miss\n");
                struct sr_arpreq *newarpreq = sr_arpcache_queuereq(&sr->cache, ip_hdr->ip_dst, packet, len, "eth1");
                handle_arpreq(sr,newarpreq);
            }
        } else {
                if (ip_proto == ip_protocol_icmp) {
        ...
        char* interface/* lent */)
{
  ...
  if (sr->nat != NULL) {
    ...
    sr->nat->ip_ext = external_interface->ip;
  }
